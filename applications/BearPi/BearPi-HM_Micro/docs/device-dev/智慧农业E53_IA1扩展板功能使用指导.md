...
# 智慧农业E53_IA1扩展板功能使用指导

**说明:**<br>
&emsp;本案例使用E53_IA1(智慧农业模块)进行开发。目前已经适配好E53接口的底层驱动放在 `device/st/drivers`路径下面,感兴趣的读者可以去翻阅一下。另外，E53模组的底层驱动也已经可以直接调用。用户层可以通过获取驱动发布的服务使用模块。<br>

**下面将讲解E53_IA1的使用教程**

&emsp;[驱动开发](#一e53ia1驱动开发)</br>
...
5. 驱动配置
    ...
    HDF框架加载驱动所需要的信息来源于HDF框架定义的驱动设备描述，因此基于HDF框架开发的驱动必须要在HDF框架定义的device_info.hcs配置文件中添加对应的设备描述，所以我们需要在device\st\bearpi_hm_micro\liteos_a\hdf_config\device_info\device_info.hcs中添加E53_IA1设备描述。 "##start##"和"##end##"之间为新增配置（"##start##"和"##end##"仅用来标识位置，添加完配置后删除这两行）
...
<div align = center><img width=400, src = "figures/E53_IA1_App.png"></div>
